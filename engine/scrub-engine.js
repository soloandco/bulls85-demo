...
function injectCSS() {
  ...
  .sw-route{--sw-route-hit:14px;--sw-route-half:7px;--sw-route-pad-y:18px;position:fixed;right:clamp(14px,2.4vw,30px);top:50%;z-index:40;transform:translateY(-50%);display:flex;flex-direction:column;gap:22px;padding:var(--sw-route-pad-y) 10px;}
  .sw-route::before{content:"";position:absolute;left:50%;top:calc(var(--sw-route-pad-y) + var(--sw-route-half));bottom:calc(var(--sw-route-pad-y) + var(--sw-route-half));width:2px;transform:translateX(-50%);background:var(--sw-accent);opacity:.28;}
  .sw-route__dot{position:relative;border:0;padding:0;margin:0;appearance:none;background:transparent;cursor:pointer;width:var(--sw-route-hit);height:var(--sw-route-hit);display:grid;place-items:center;}
  ...
  @media (hover:none) and (pointer:coarse){
    .sw-route{--sw-route-hit:28px;--sw-route-half:14px;--sw-route-pad-y:14px;padding:var(--sw-route-pad-y) 6px;}
